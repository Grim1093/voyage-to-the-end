...
export default function EventHub() {
    ...
    return (
        <main className="flex flex-col items-center relative overflow-hidden min-h-screen bg-transparent">
            ...
            <motion.div variants={staggerContainer} initial="hidden" animate="show" className="max-w-4xl w-full z-20 flex flex-col items-center pb-12 pt-16 px-4 relative">
                <motion.div variants={itemVariant} className="text-center mb-16 space-y-4">
                    ...
                    {(formattedDate || event.location) && (
                        <div className="flex flex-wrap items-center justify-center gap-4 text-[10px] text-[var(--tenant-text)] opacity-90 mt-6 font-medium uppercase tracking-widest">
                            {formattedDate && (
                                <span className="flex items-center gap-2 px-4 py-2 bg-black/30 border border-white/10 shadow-lg backdrop-blur-md" style={{ borderRadius: 'var(--tenant-btn-radius)' }}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 w-full max-w-2xl">
                    <motion.div variants={itemVariant}>
                        ...
                        >
                            <style jsx>{`div.group:hover { box-shadow: 0 0 40px color-mix(in srgb, var(--hover-shadow) 20%, transparent); }`}</style>
                            ...
                            <div className="w-12 h-12 bg-white/[0.05] flex items-center justify-center mb-6 border border-white/[0.1] relative z-10 shadow-inner" style={{ borderRadius: 'var(--tenant-btn-radius)' }}>
                            ...
                            <Link href={`/${eventSlug}/register`} className="w-full py-3 px-4 font-bold text-[11px] uppercase tracking-wider transition-all relative z-10 shadow-[0_0_20px_rgba(255,255,255,0.1)] active:scale-95 text-[var(--tenant-bg)] hover:opacity-90" style={{ backgroundColor: 'var(--tenant-text)', borderRadius: 'var(--tenant-btn-radius)' }}>
                                Start Registration
                            </Link>
                        </div>
                    </motion.div>
                    ...
                    <motion.div variants={itemVariant}>
                        ...
                        >
                            ...
                            <Link href={`/${eventSlug}/portal`} className="w-full py-3 px-4 bg-white/[0.05] hover:bg-white/[0.15] border border-white/[0.1] font-bold text-[11px] uppercase tracking-wider transition-all relative z-10 active:scale-95" style={{ color: 'var(--tenant-text)', borderRadius: 'var(--tenant-btn-radius)' }}>
                                Enter Portal
                            </Link>
                        </div>
                    </motion.div>
