    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-8 text-left p-4 sm:p-8">
            ...
            {status === 'success' && (
                <div className="flex flex-col p-6 bg-emerald-500/5 border border-emerald-500/10 rounded-3xl shadow-[0_0_20px_rgba(16,185,129,0.05)] text-center">
                    <div className="mx-auto flex items-center justify-center h-10 w-10 rounded-full bg-emerald-500/10 mb-3 border border-emerald-500/20">
                        <svg className="h-5 w-5 text-emerald-500 dark:text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                    </div>
                    <p className="text-sm font-semibold text-emerald-600 dark:text-emerald-400 mb-1">{message}</p>
                    <p className="text-[11px] text-emerald-600/70 dark:text-emerald-500/60 tracking-wide font-medium">Check your email for your 6-character access code.</p>
                </div>
            )}

            {/* Input Grid - Optimized for dynamic theme aesthetic */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                <div className="space-y-2">
                    <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em] ml-2">Full Name</label>
                ...
                <div className="space-y-2">
                    <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em] ml-2">Email Address</label>
                ...
                <div className="space-y-2">
                    <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em] ml-2">Phone Number</label>
                ...
                <div className="space-y-2">
                    <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em] ml-2">ID Number</label>
                    <input type="text" name="idNumber" value={formData.idNumber} onChange={handleChange} 
                        className="w-full px-5 py-3.5 bg-black/[0.02] dark:bg-white/[0.02] backdrop-blur-sm border border-border rounded-full focus:ring-1 focus:ring-primary/50 focus:border-primary/30 outline-none transition-all duration-300 shadow-inner hover:bg-black/[0.04] dark:hover:bg-white/[0.04] text-foreground placeholder-muted-foreground/50 font-mono text-sm tracking-wider" 
                        placeholder="Passport / Govt ID" disabled={status === 'loading'} />
                </div>

                <div className="md:col-span-2 space-y-2">
                    <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em] ml-2">Identity Document URL</label>
                ...
                <div className="md:col-span-2 space-y-2">
                    <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em] ml-2">Dietary Restrictions</label>
            ...
            <button type="submit" disabled={status === 'loading'} 
                className="w-full mt-10 bg-foreground text-background hover:bg-foreground/80 font-bold py-4 px-6 rounded-full shadow-[0_0_25px_rgba(37,99,235,0.1)] transform transition-all duration-300 active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed flex justify-center items-center group text-xs uppercase tracking-[0.2em]">
