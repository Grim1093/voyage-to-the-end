    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-8 text-left p-4 sm:p-8">
            ...
            {status === 'error' && (
                ...
                >
                    <svg className="w-4 h-4 text-rose-500 mr-3 mt-0.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
            ...
            {status === 'success' && (
                <div 
                    className="flex flex-col p-6 bg-emerald-500/10 border border-emerald-500/20 shadow-[0_0_20px_rgba(16,185,129,0.05)] text-center"
                ...
                >
                    ...
                    <p className="text-sm font-bold text-emerald-400 mb-1">{message}</p>
                    <p className="text-[11px] text-emerald-500/80 tracking-wide font-medium">Check your email for your 6-character access code.</p>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                <div className="space-y-2">
                    <label className="block text-[10px] font-bold text-[var(--tenant-text)] opacity-60 uppercase tracking-[0.2em] ml-2">Full Name</label>
                    <input type="text" name="fullName" value={formData.fullName} onChange={handleChange} 
                        className="w-full px-5 py-3.5 bg-black/40 backdrop-blur-sm border border-white/[0.1] focus:outline-none transition-all duration-300 shadow-inner hover:bg-black/60 text-[var(--tenant-text)] text-sm focus:ring-1 focus:ring-[var(--tenant-primary)] focus:border-[var(--tenant-primary)] placeholder-white/30" 
                        style={{ borderRadius: 'var(--tenant-btn-radius)' }}
                        placeholder="Satoshi Nakamoto" disabled={status === 'loading'} />
                </div>

                <div className="space-y-2">
                    <label className="block text-[10px] font-bold text-[var(--tenant-text)] opacity-60 uppercase tracking-[0.2em] ml-2">Email Address</label>
                ...
                <div className="space-y-2">
                    <label className="block text-[10px] font-bold text-[var(--tenant-text)] opacity-60 uppercase tracking-[0.2em] ml-2">Phone Number</label>
                ...
                <div className="space-y-2">
                    <label className="block text-[10px] font-bold text-[var(--tenant-text)] opacity-60 uppercase tracking-[0.2em] ml-2">ID Number</label>
                    <input type="text" name="idNumber" value={formData.idNumber} onChange={handleChange} 
                        className="w-full px-5 py-3.5 bg-black/40 backdrop-blur-sm border border-white/[0.1] focus:outline-none transition-all duration-300 shadow-inner hover:bg-black/60 text-[var(--tenant-text)] font-mono text-sm tracking-wider focus:ring-1 focus:ring-[var(--tenant-primary)] focus:border-[var(--tenant-primary)] placeholder-white/30" 
                        style={{ borderRadius: 'var(--tenant-btn-radius)' }}
                        placeholder="Passport / Govt ID" disabled={status === 'loading'} />
                </div>

                <div className="md:col-span-2 space-y-2">
                    <label className="block text-[10px] font-bold text-[var(--tenant-text)] opacity-60 uppercase tracking-[0.2em] ml-2">Identity Document URL</label>
                ...
                <div className="md:col-span-2 space-y-2">
                    <label className="block text-[10px] font-bold text-[var(--tenant-text)] opacity-60 uppercase tracking-[0.2em] ml-2">Dietary Restrictions</label>
            ...
            <button type="submit" disabled={status === 'loading'} 
                className="w-full mt-10 font-bold py-4 px-6 shadow-[0_0_25px_rgba(255,255,255,0.1)] transform transition-all duration-300 active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed flex justify-center items-center group text-xs uppercase tracking-[0.2em]"
